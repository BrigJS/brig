...
Application.prototype.loadModule = function(m) {
	...
	this.engine.addModulePath(m.importPath);

	// Create types
	if (m.types instanceof Array) {
		m.types.forEach(function(typeInit) {
			var defInfo = typeInit(this.brig);
			var type = this.engine.createType(defInfo.typeName, defInfo.implement);

			defInfo.onRegistered(type);
		}.bind(this));
	}
};
